...
local BFS = import(".findPath.BreadthFirstSearch")
local Dijkstra = import(".findPath.Dijkstra")
local GBFS = import(".findPath.GreedyBestFirstSearch")
local BStar = import(".findPath.BStar")
...
function GameScene:loadPlayer()
    ...
    self.player:setSpawnPoint(spawnPoint)
...
function GameScene:loadOtherUI()
    ...
    self.targetIcon = cc.Sprite:create("quanxinMap/images/arrow.png")
    self:addChild(self.targetIcon)
    ...
    goBtn:addClickEventListener(function()
        ...
        self.targetIcon:setPosition(self.mapNode:tp2cp(goal))
...
function GameScene:loadTree(...)
    local avatars = {hero = self.player, monsters = self.monsters}
...
function GameScene:onUpdate(dt)
   ...
   if self.isOpenAI then
        ...
        if acc > 1 then
            ...
            if ret == "SUCCESS" then
                print("Hero TREE is RUN AROUND")
            end
            local monstersTree = self.tree:getMonstersTree()
            for _, monsterTree in pairs(monstersTree) do
                monsterTree:run()
            end
        end
   end
...
function GameScene:onTouchEnded(touch, event)
    ...
    self.targetIcon:setPosition(touchLocation)
...
function GameScene:findPath(start, goal)
    ...
    local cameFromPath = AStar:findPath(self.mapNode, start, goal) -- A Star
