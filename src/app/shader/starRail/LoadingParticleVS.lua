...
#define NUM_SEGMENT 5  // 使用宏定义常量

uniform vec3 bezierPos[NUM_SEGMENT];  // 使用常量定义数组大小
...
void main(){
  ...
  vec3 newPosition = np + +vec3(fun1(a_position.xy,nowTime*15.)*rnd.z,0.)+mix(start,end,sin(nowTime*6.)*.5+.5)*mix(.2,1.5,smoothstep(.3,1.,nowTime+rnd.y));

  
  vUv = fun1(a_texCoord - .5, start.z+nowTime)+.5;
  ...
  vec4 mvPosition = CC_MVMatrix * vec4(newPosition,1.);
  gl_Position = CC_PMatrix * mvPosition;
}
]]